...
name: herdr
description:
  "Control Herdr, a terminal multiplexer for coding agents. Use when the user explicitly mentions
  Herdr, asks to inspect or control panes, tabs, workspaces, commands, or another agent, or asks to
  create, open, or remove a git worktree — Herdr owns worktree lifecycle when HERDR_ENV=1. Do not
  use merely because a task could benefit from a background terminal, delegation, or parallel work.
